...
public class WarGame implements Game{
	...
	private void pickSortieOutOf3() {
		...
		do {
			...
			try {
				...
				p1sortie = p1ready[p1 - 1];
				p1ready[p1 - 1] = null;
				
				if(turn < 23) {
					p1ready[p1 - 1] = p1back.get(0);
					p1back.remove(0);
				}
				else if(p1 == 1 || p1 == 2) {
					p1ready[p1 - 1] = p1ready[2];
					p1ready[2] = null;
				}
				...
				p2sortie = p2ready[p2 - 1];
				p2ready[p2 - 1] = null;
				
				if(turn < 23) {
					p2ready[p2 - 1] = p2back.get(0);
					p2back.remove(0);
				}
				else if(p2 == 1 || p2 == 2) {
					p2ready[p2 - 1] = p2ready[2];
					p2ready[2] = null;
				}
	...
	private void pickSortieOutOf2() {
		...
		do {
			...
			try {
				...
				if(ex.getMessage() == null)
					System.out.println("잘못된 입력입니다: 1~2사이의 값을 넣어주세요.\n");
				else
					System.out.printf("잘못된 입력입니다: %s\n", ex.getMessage());
				in.nextLine();
			}
			catch(IndexOutOfBoundsException ex) {//범위 밖의 정수를 입력하면
			System.out.printf("잘못된 입력입니다: 1~2사이의 값을 입력해주세요.\n");
			//ex.printStackTrace();
			}
		} while(pickthesortie);
	}
		...
		if(p1Num > p2Num) {
			...
			if(turn == 25) {
				...
				if(isWar==1) {
					...
					System.out.println("세 번째 무승부입니다.각자의 카드를 가져갑니다.");
					p1front+=3;p2front+=3;
					isWar=0;
				}
			}
	...
	private void sortCards() {
		System.out.println("남은 카드들을 앞 번호로 정렬합니다.");
		if(p1ready[1] == null && p1ready[2] != null) {
			p1ready[1] = p1ready[2];
			p1ready[2] = null;
		}
		if(p2ready[1] == null && p2ready[2] != null) {
			p2ready[1] = p2ready[2];
			p2ready[2] = null;
		}
		if(p1ready[0] == null && p1ready[1] != null) {
			p1ready[0] = p1ready[1];
			p1ready[1] = null;
		}
		if(p2ready[0] == null && p2ready[1] != null) {
			p2ready[0] = p2ready[1];
			p2ready[1] = null;
		}
	}
	
	private void nullChecker(int phase) {
		if(phase == 3) {
			for(int i = 0; i < 3; i++) {
				if(p1ready[i] == null) {
					p1ready[i] = p1back.get(0);
